...
module tb_check_mem;
    ...
    // 直接访问 RAM 内部
    wire [7:0] mem_0 = u_ram.mem[0];
    wire [7:0] mem_1 = u_ram.mem[1];
    wire [7:0] mem_2 = u_ram.mem[2];
    wire [7:0] mem_11 = u_ram.mem[11];
    wire [7:0] mem_12 = u_ram.mem[12];
    ...
    initial begin
        ...
        $display("\n启动格式化器前的 RAM 内容:");
        $display("  mem[0] = 0x%02x (%c)", mem_0, mem_0);
        $display("  mem[1] = 0x%02x (%c)", mem_1, mem_1);
        $display("  mem[2] = 0x%02x (%c)", mem_2, mem_2);
        $display("  mem[11] = 0x%02x (%c)", mem_11, mem_11);
        $display("  mem[12] = 0x%02x (%c)", mem_12, mem_12);
