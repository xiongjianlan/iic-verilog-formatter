...
module tb_debug_addr2;
    ...
    always @(posedge clk) begin
        if (sel_formatter && u_formatter.dst_wr_en) begin
            $display("T=%0t: 写入 ram_addr_a=%0d ram_wr_data=0x%02x dst_wr_addr=%0d dst_ptr=%0d", 
                     $time, ram_addr_a, ram_wr_data, u_formatter.dst_wr_addr, u_formatter.dst_ptr);
        end
    end
    ...
    initial begin
        ...
        for (i = 0; i < fmt_result_len; i = i + 1) begin
            ...
            @(posedge clk); $display("  [%0d] = 0x%02x (%c)", i, ram_dout, ram_dout);
        end
