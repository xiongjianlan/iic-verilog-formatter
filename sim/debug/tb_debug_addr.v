...
module tb_debug_addr;
    ...
    // 中间信号
    wire [11:0] src_rd_addr = u_formatter.src_rd_addr;
    wire [11:0] dst_wr_addr = u_formatter.dst_wr_addr;
    wire src_rd_en = u_formatter.src_rd_en;
    wire dst_wr_en = u_formatter.dst_wr_en;
    ...
    always @(posedge clk) begin
        if (sel_formatter) begin
            $display("T=%0t state=%0d src_rd_en=%b src_rd_addr=%0d dst_wr_en=%b dst_wr_addr=%0d ram_addr_a=%0d ram_dout=0x%02x", 
                     $time, u_formatter.state, src_rd_en, src_rd_addr, dst_wr_en, dst_wr_addr, ram_addr_a, ram_dout);
        end
    end
